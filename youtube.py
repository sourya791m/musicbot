...
class YouTube:
    def __init__(self):
        ...
        # Get API settings dynamically from config
    ...
    def api_url(self) -> str:
        """Dynamically fetch the active YouTube API URL from global config."""
        return config.YOUTUBE_API_URL
    ...
    async def download_via_api(self, link: str, video: bool = False) -> Optional[str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # ADD THE NGROK BYPASS HEADER HERE
                headers = {
                ...
                async with session.get(
                    ...
                    params=params,
                    headers=headers,  # <--- Pass the header here
                    ...
                    if 'application/json' in content_type:
                        ...
                        # ADD THE BYPASS HEADER HERE AS WELL
                        stream_headers = {
                            "ngrok-skip-browser-warning": "69420"
                        }
                        async with session.get(
                            stream_url, 
                            headers=stream_headers, # <--- Add this line
